...
const WeeklyAccount = () => {
  ...
  // Investment data with weekly collections
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const data = [
    { 
      id: 1,
      WeeklyCollection: 1000, 
      rate: '6.0%', 
      days1: 365, 
      days2: 730, 
      days3: 1095, 
      ...
      amount2: 155107.64, 
      amount3: 239694.46,
      weeklyBreakdown: Array.from({ length: 156 }, (_, i) => ({
        week: i + 1,
        amount: 1000,
        date: new Date(2025, 0, 1 + (i * 7)).toISOString().split('T')[0],
        weekNumber: `2025-W${String(Math.floor(i / 4) + 1).padStart(2, '0')}`
      }))
    },
    { 
      id: 2,
      WeeklyCollection: 1500, 
      rate: '6.5%', 
      days1: 365, 
      days2: 730, 
      days3: 1095, 
  ...
  // Get weekly collection data for selected principal
  const getWeeklyData = (principal) => {
    const investment = data.find(d => d.WeeklyCollection === principal);
    if (!investment) return [];
    return investment.weeklyBreakdown;
  };

  const getFilteredWeeks = (period) => {
    if (period === 'all') return 156; // 3 years = 156 weeks
    return Math.floor(period / 7);
  };
  ...
  const totalWeeklyCollection = data.reduce((sum, row) => sum + row.WeeklyCollection, 0);

  const getCAGR = (start, end, years) => {
    return ((Math.pow(end / start, 1 / years) - 1) * 100).toFixed(1);
  };

  // Get weekly collection data for selected investment
  const weeklyData = selectedPrincipal ? getWeeklyData(selectedPrincipal) : [];
  const allDisplayedWeeks = activeTab === 'all' ? weeklyData : weeklyData.slice(0, getFilteredWeeks(activeTab));
  const totalWeeklyAmount = allDisplayedWeeks.reduce((sum, d) => sum + d.amount, 0);

  // Pagination logic
  const totalPages = Math.ceil(allDisplayedWeeks.length / itemsPerPage);
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const displayedWeeks = allDisplayedWeeks.slice(indexOfFirstItem, indexOfLastItem);

  // Calculate totals for displayed page
  const pageTotalAmount = displayedWeeks.reduce((sum, d) => sum + d.amount, 0);

  // Calculate maturity amounts for selected investment
  const selectedInvestmentData = selectedPrincipal ? data.find(d => d.WeeklyCollection === selectedPrincipal) : null;
  ...
  const openApplicationModal = (investment) => {
    ...
    setFormData(prev => ({
      ...prev,
      investmentAmount: investment ? investment.WeeklyCollection : '',
      investmentType: 'Weekly'
    }));
        ...
        <div className="container relative px-4 py-10 mx-auto md:pt-8 md:pb-24">
          <div className="max-w-3xl mx-auto text-center text-white">
            ...
            <h1 className="mb-2 text-3xl font-bold md:text-4xl lg:text-5xl">
              Investment Weekly Tracker
            </h1>
            <p className="mb-3 text-base text-white/90 md:text-lg">
              Earn up to <span className="text-2xl font-bold">8.0% p.a.*</span> Track weekly collections across different investment scenarios with 1Y, 2Y, and 3Y projections.
            </p>
          </div>
        </div>
          ...
          <div className="relative flex flex-col gap-6 p-8 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h1 className="text-4xl font-bold sm:text-5xl bg-gradient-to-r from-[#228296] via-[#4a2c7a] to-[#6f3c85] bg-clip-text text-transparent">
                Track Your Weekly Investments
              </h1>
              <p className="max-w-2xl mt-3 text-sm text-slate-600 dark:text-slate-400">
                Track weekly collections across different investment scenarios with 1Y, 2Y, and 3Y projections.
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-5">
          <div className="p-6 transition-all duration-300 border shadow-lg bg-slate-50 dark:bg-slate-900 rounded-2xl border-slate-200/50 dark:border-slate-700/50 hover:shadow-xl hover:scale-105">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold tracking-wider uppercase text-slate-500 dark:text-slate-400">Weekly Collection</p>
                <p className="mt-2 text-xl font-bold text-slate-900 dark:text-slate-100">₹{totalWeeklyCollection.toLocaleString()}</p>
              </div>
          ...
          <div className="p-6 transition-all duration-300 border shadow-lg bg-slate-50 dark:bg-slate-900 rounded-2xl border-slate-200/50 dark:border-slate-700/50 hover:shadow-xl hover:scale-105">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-xl font-bold text-emerald-600 dark:text-emerald-400">₹{totalAmount1.toFixed(2)}</p>
              </div>
          ...
          <div className="p-6 transition-all duration-300 border shadow-lg bg-slate-50 dark:bg-slate-900 rounded-2xl border-slate-200/50 dark:border-slate-700/50 hover:shadow-xl hover:scale-105">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-xl font-bold text-purple-600 dark:text-purple-400">₹{totalAmount2.toFixed(2)}</p>
              </div>
          ...
          <div className="p-6 transition-all duration-300 border shadow-lg bg-slate-50 dark:bg-slate-900 rounded-2xl border-slate-200/50 dark:border-slate-700/50 hover:shadow-xl hover:scale-105">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-xl font-bold text-amber-600 dark:text-amber-400">₹{totalAmount3.toFixed(2)}</p>
              </div>
          ...
          <div className="p-6 transition-all duration-300 border shadow-lg bg-slate-50 dark:bg-slate-900 rounded-2xl border-slate-200/50 dark:border-slate-700/50 hover:shadow-xl hover:scale-105">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold tracking-wider uppercase text-slate-500 dark:text-slate-400">CAGR (3Y)</p>
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          {[1000, 1500, 2000, 2500, 3000].map(amount => (
            <button
              key={amount}
              onClick={() => {
                setSelectedPrincipal(selectedPrincipal === amount ? null : amount);
                setCurrentPage(1);
              }}
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
                  {selectedPrincipal ? `Showing ₹${selectedPrincipal}/week investments • ` : 'Showing all investments • '}
                  {allDisplayedWeeks.length} weeks displayed
                </p>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-full bg-[#228296]/10 text-[#228296] dark:bg-[#228296]/20 dark:text-[#6f3c85]">
                  ...
                  {allDisplayedWeeks.length} records
                </div>
                <div className="px-3 py-1.5 text-xs font-medium rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                  Total: ₹{totalWeeklyAmount.toLocaleString()}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="px-6 pt-4 pb-8 overflow-x-auto">
            {displayedWeeks.length > 0 ? (
                ...
                <table className="min-w-full">
                  <thead>
                    <tr className="bg-gradient-to-r from-[#228296] to-[#6f3c85] rounded-xl shadow-lg">
                      <th className="px-6 py-4 text-xs font-semibold tracking-wider text-left text-white uppercase rounded-tl-xl">#</th>
                      <th className="px-6 py-4 text-xs font-semibold tracking-wider text-left text-white uppercase">Week</th>
                      <th className="px-6 py-4 text-xs font-semibold tracking-wider text-left text-white uppercase">Week Number</th>
                      <th className="px-6 py-4 text-xs font-semibold tracking-wider text-left text-white uppercase">Date</th>
                      <th className="px-6 py-4 text-xs font-semibold tracking-wider text-left text-white uppercase">Amount (₹)</th>
                      <th className="px-6 py-4 text-xs font-semibold tracking-wider text-left text-white uppercase">Maturity (₹)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                    {displayedWeeks.map((week, index) => {
                      // Calculate maturity amount for this week (cumulative)
                      const cumulativeAmount = displayedWeeks
                        .slice(0, index + 1)
                      ...
                      if (selectedInvestmentData) {
                        const dailyRate = parseFloat(selectedInvestmentData.rate) / 100 / 365;
                        const days = week.week * 7;
                        maturityAmount = cumulativeAmount * Math.pow(1 + dailyRate, days);
                      }
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-sm font-medium text-slate-900 dark:text-slate-100">Week {week.week}</td>
                          <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{week.weekNumber}</td>
                          <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{week.date}</td>
                          ...
                          <td className="px-6 py-4">
                            <span className="inline-block px-4 py-1.5 text-sm font-semibold rounded-full bg-purple-50 dark:bg-purple-950/30 text-purple-700 dark:text-purple-400 shadow-sm">
                              ₹{selectedInvestmentData ? maturityAmount.toFixed(2) : 'N/A'}
                            </span>
                          </td>
                        </tr>
                      );
                  ...
                  <tfoot>
                    <tr className="bg-gradient-to-r from-[#228296]/20 to-[#6f3c85]/20 border-t-2 border-slate-300 dark:border-slate-600">
                      <td colSpan="4" className="px-6 py-4 text-sm font-bold text-right text-slate-700 dark:text-slate-300">
                        Page Total:
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className="inline-block px-4 py-1.5 text-sm font-bold rounded-full bg-purple-100 dark:bg-purple-900/40 text-purple-800 dark:text-purple-300">
                          ₹{selectedInvestmentData ? (pageTotalAmount * (1 + parseFloat(selectedInvestmentData.rate) / 100)).toFixed(2) : 'N/A'}
                        </span>
                      </td>
                    </tr>
                    <tr className="bg-gradient-to-r from-[#228296]/5 to-[#6f3c85]/5 border-t border-slate-200 dark:border-slate-700">
                      <td colSpan="4" className="px-6 py-4 text-sm font-bold text-right text-slate-800 dark:text-slate-200">
                        Grand Total:
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className="inline-block px-4 py-2 text-sm font-bold text-purple-900 bg-purple-200 rounded-full shadow-md dark:bg-purple-800/40 dark:text-purple-200">
                          ₹{selectedInvestmentData ? (totalWeeklyAmount * (1 + parseFloat(selectedInvestmentData.rate) / 100)).toFixed(2) : 'N/A'}
                        </span>
                      </td>
                    </tr>
                  </tfoot>
                  ...
                  <div className="flex flex-col items-center justify-between gap-4 pt-6 mt-4 border-t border-slate-200 dark:border-slate-700 md:flex-row">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-slate-600 dark:text-slate-400">
                        {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, allDisplayedWeeks.length)} of {allDisplayedWeeks.length}
                      </span>
                    </div>
              ...
              <div className="py-12 text-center">
                <p className="text-lg text-slate-500 dark:text-slate-400">No data available</p>
                <p className="mt-2 text-sm text-slate-400 dark:text-slate-500">Select an investment amount to view weekly details</p>
              </div>
            )}
          </div>
        </div>
          ...
          {data.map((item) => (
            <div 
              ...
              className={`p-6 border shadow-lg bg-slate-50 dark:bg-slate-900 rounded-2xl border-slate-200/50 dark:border-slate-700/50 transition-all duration-300 cursor-pointer ${
                selectedPrincipal === item.WeeklyCollection ? 'ring-2 ring-[#228296] shadow-xl scale-105' : 'hover:shadow-xl hover:scale-105'
              }`}
              onClick={() => {
                setSelectedPrincipal(selectedPrincipal === item.WeeklyCollection ? null : item.WeeklyCollection);
                setCurrentPage(1);
              }}
            >
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-sm font-semibold text-[#228296] dark:text-[#6f3c85]">{item.rate}</p>
                  <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">₹{item.WeeklyCollection}</p>
              ...
              <div className="grid grid-cols-3 gap-4 pt-3 mt-3 border-t border-slate-200 dark:border-slate-700">
                <div className="text-center">
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400">1 Year</p>
                  <p className="text-sm font-bold text-emerald-600 dark:text-emerald-400">₹{item.amount1.toFixed(0)}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400">2 Years</p>
                  <p className="text-sm font-bold text-purple-600 dark:text-purple-400">₹{item.amount2.toFixed(0)}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400">3 Years</p>
                  <p className="text-sm font-bold text-amber-600 dark:text-amber-400">₹{item.amount3.toFixed(0)}</p>
                </div>
              </div>
              <button
                onClick={(e) => {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="p-3 rounded-2xl bg-gradient-to-br from-[#228296]/20 to-[#6f3c85]/20 shadow-inner">
              <svg className="w-6 h-6 text-[#228296] dark:text-[#6f3c85]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v1m0 1v1m0 1v1m0 1v1" />
              </svg>
            </div>
            <div>
              ...
              {selectedInvestment && (
                <p className="text-sm text-slate-600 dark:text-slate-400 mt-0.5">
                  <span className="font-semibold text-[#228296] dark:text-[#6f3c85]">
                    ₹{selectedInvestment.WeeklyCollection}
                  </span>
                  /week •{" "}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                  Investment Amount <span className="text-[#6f3c85]">(₹)</span>
                </label>
                <input
                  type="number"
            ...
            <div className="p-5 rounded-2xl bg-gradient-to-r from-[#228296]/10 to-[#6f3c85]/10 border border-[#228296]/20 dark:border-[#6f3c85]/20">
              <div className="flex items-center gap-2 mb-3">
                <div className="p-1.5 rounded-lg bg-[#228296]/20 dark:bg-[#6f3c85]/20">
                  <svg className="w-4 h-4 text-[#228296] dark:text-[#6f3c85]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3 border bg-white/60 dark:bg-slate-800/60 rounded-xl border-white/50 dark:border-slate-700/50">
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Weekly Collection</p>
                  <p className="text-lg font-bold text-[#228296]">₹{selectedInvestment.WeeklyCollection}</p>
                </div>
                <div className="p-3 border bg-white/60 dark:bg-slate-800/60 rounded-xl border-white/50 dark:border-slate-700/50">
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Interest Rate</p>
                  <p className="text-lg font-bold text-[#6f3c85]">{selectedInvestment.rate}</p>
                </div>
                <div className="p-3 border bg-white/60 dark:bg-slate-800/60 rounded-xl border-white/50 dark:border-slate-700/50">
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400">3Y Projected</p>
                  <p className="text-lg font-bold text-amber-600 dark:text-amber-400">₹{selectedInvestment.amount3?.toFixed(0) || 'N/A'}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex items-start gap-3 p-3 border rounded-xl bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700">
            ...
            <label htmlFor="terms" className="text-sm leading-relaxed text-slate-600 dark:text-slate-300">
              I agree to the{" "}
              <a href="#" className="text-[#228296] hover:underline dark:text-[#6f3c85] font-medium transition-colors duration-200">
