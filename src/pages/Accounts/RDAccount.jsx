...
const RDAccount = () => {
  ...
  const features = [
    {
      ...
      description: "Earn up to 8.0% p.a. with monthly compounding",
      color: "from-emerald-500 to-emerald-600"
    },
    {
      icon: <Calendar className="w-6 h-6" />,
      title: "Monthly Deposits",
      description: "Fixed monthly savings that grow into a large corpus",
    ...
    },
    {
      ...
      description: "Safe and regulated recurring deposit option",
  ...
  const benefits = [
    ...
    },
    {
      ...
      description: "Automatic monthly transfers for consistency",
      stat: "Set & Forget"
    },
    {
  ...
  const faqs = [
    {
      question: "What is the minimum monthly deposit?",
      answer: "The minimum monthly deposit is ₹500, making it accessible for everyone to start saving. You can increase your deposit amount at any time."
    },
    {
      question: "Can I increase my monthly deposit later?",
      answer: "Yes, you can increase your monthly deposit at any time to accelerate your savings growth. Simply update your deposit amount through your account settings."
    },
    {
    ...
    },
    {
      ...
      answer: "Interest is calculated monthly and compounded quarterly, helping your savings grow faster. The interest rate varies based on the investment period you choose."
    },
    {
      question: "Can I have multiple RD accounts?",
      answer: "Yes, you can open multiple RD accounts for different savings goals. Each account can have its own monthly deposit amount and tenure."
    }
  ];
  ...
  const calculateMaturity = () => {
    ...
    // Compound interest formula for RD: A = P * (((1 + r/n)^(nt) - 1) / (r/n)) * (1 + r/n)
    const n = 4; // Quarterly compounding
    const r = rate / n;
    const nt = n * years;
    const maturity = monthlyAmount * ((Math.pow(1 + r, nt) - 1) / r) * (1 + r);
  ...
  const handleCalculate = () => {
    if (monthlyAmount < 500) {
      toast.error('Minimum monthly deposit is ₹500');
      return;
    }
    if (monthlyAmount > 50000) {
      toast.error('Maximum monthly deposit is ₹50,000');
      return;
    }
    calculateMaturity();
  };
  ...
  const handleSubmit = (e) => {
    ...
    toast.success('Account opened successfully! Welcome to RD savings.');
    setShowModal(false);
    resetForm();
  };
  ...
  const testimonials = [
    {
      ...
      quote: "RD helped me save ₹1,00,000 in just 2 years without any hassle!",
      rating: 5
    },
    {
      name: "Sneha Patel",
      role: "Teacher",
      quote: "The monthly compounding feature is excellent. My savings grew more than expected.",
      rating: 5
    }
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-800 dark:via-slate-800 dark:to-slate-800 text-slate-900 dark:text-slate-100">
      <div className="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
          ...
          <div className="relative flex flex-col gap-6 p-8 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="max-w-2xl mt-3 text-sm text-slate-600 dark:text-slate-400">
                A disciplined savings plan with fixed monthly deposits and attractive interest rates to build a substantial corpus over time.
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-4">
          {[
            { label: "Min Monthly Deposit", value: "₹500", icon: Wallet, color: "emerald" },
            { label: "Max Interest Rate", value: "8.0% p.a.", icon: Percent, color: "blue" },
            { label: "Tenure", value: "1-5 Years", icon: Calendar, color: "purple" },
            { label: "Compounding", value: "Quarterly", icon: TrendingUp, color: "amber" }
          ].map((stat, index) => (
            <div 
              key={index}
            ...
            <div className={`mt-6 p-6 border rounded-2xl bg-gradient-to-r from-[#228296]/5 to-[#6f3c85]/5 border-slate-200 dark:border-slate-700 transition-all duration-500 ${
              ...
              <div className="mt-4 text-center">
                <p className="text-sm text-slate-600 dark:text-slate-400">
                  Based on ₹{monthlyAmount}/month for {calculatedMaturity.years} year{calculatedMaturity.years > 1 ? 's' : ''} @ {calculatedMaturity.rate}% p.a.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="relative p-8 overflow-hidden border shadow-xl bg-gradient-to-r from-[#228296] to-[#6f3c85] rounded-3xl border-slate-200/50">
          ...
          <div className="relative flex flex-col items-center justify-between gap-6 md:flex-row">
            <div>
              ...
              <p className="text-white/80">Open your RD account today and start building your wealth.</p>
            </div>
      ...
      {/* Open Account Modal - Same as DRD but with RD specific fields */}
      {showModal && (
                ...
                {modalStep === 2 && (
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Monthly Deposit Amount (₹)</label>
                        <div className="relative">
                          <Wallet className="absolute left-3 top-2.5 w-5 h-5 text-slate-400" />
                          <input type="number" name="monthlyDeposit" value={formData.monthlyDeposit} onChange={handleInputChange} min="500" max="50000" className="w-full pl-10 pr-4 py-2.5 border rounded-lg bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#228296]" placeholder="Monthly deposit amount" />
                        </div>
                      </div>
                      <div>
                    ...
                    <div className="p-4 border rounded-lg bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-700">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-amber-700 dark:text-amber-400">By opening this account, you agree to the terms and conditions of the RD scheme. Your deposits are secure and regulated by financial authorities.</p>
                        </div>
                      </div>
                    </div>
